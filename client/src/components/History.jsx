...
export default function History({ onLoadSession, currentSessionId, onClearActiveSession }) {
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      ...
      // If we deleted the active session, clear it in the main App
      if (id === currentSessionId && onClearActiveSession) {
  ...
  const handleClearAll = async () => {
    ...
    try {
      ...
      // Clear active session since all are gone
      if (onClearActiveSession) {
        onClearActiveSession();
      }
    } catch (error) {
          ...
          <div className="flex flex-col gap-3">
            {sessions.map(session => (
              <button
                ...
                className="flex flex-col text-left px-6 py-4 bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all group relative overflow-hidden"
              >
                <div className="flex justify-between items-start w-full mb-2 pr-8">
                  <h3 className="font-semibold text-[#1E293B] text-[15px] group-hover:text-[#002A5C] transition-colors line-clamp-1">
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100"
